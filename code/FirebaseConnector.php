...
/**
 * Firebase connector class using the Firebase specific api
 *
 ...
 * @subpackage model
 */
class FirebaseConnector extends DBConnector
{
    /**
    ...
    public function connect($parameters, $selectDB = false)
    {
        ...
        if ($this->dbConn === false) {
            ...
            if ($error && preg_match('/function\\.pg-connect\\<\\/a\\>\\]\\: (?<message>.*)/', $error['message'], $matches)) {
                ...
                $this->databaseError("Couldn't connect to Firebase database.");
            }
        ...
        $this->databaseName = empty($parameters['database']) ? FirebaseDatabase::MASTER_DATABASE : $parameters['database'];
    }
    ...
    public function preparedQuery($sql, $parameters, $errorLevel = E_USER_ERROR)
    {
        ...
        return new FirebaseQuery($result);
    }
    ...
    public function selectDatabase($name)
    {
        if ($name !== $this->databaseName) {
            user_error("FirebaseConnector can't change databases. Please create a new database connection", E_USER_ERROR);
        }
        return true;
    }
